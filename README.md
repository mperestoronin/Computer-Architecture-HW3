...
Пользователь указывает "размеры" библиотеки. Главный поток создаёт портфель, формирует список задач, а также ставит цель - заполнить всю библиотеку (максимальное количество задач). После чего студенты (потоки) начинают возвращать книги в библиотеку и писать об этом в отчете. Поток считывает информацию из портфель задач и повышает его счетчик:
``` C++
arguments->briefcase += 1;
```
затем заполняет каталог по строке старого номера портфеля задач с ипользовованием списка задач, содержащего координаты книги. Студент (Поток) останавливает только тогда, когда портфель задач оказывается равен лимиту по задачам.
